		...
		function process_order( $order_id ) {
			...
			try {
				...
				if( isset( $payment_form["opayo_card_token"] ) && is_numeric( $payment_form["opayo_card_token"] ) ) {
					...
					// No token ;/
					throw new Exception( __( 'No Token ', 'woocommerce-gateway-sagepay-form' ) . '<pre>' . print_r( $payment_form, TRUE ) . '</pre><pre>' . print_r( $_REQUEST, TRUE ) . '</pre>' );
				}
				...
				if( is_wp_error( $result ) ) {
					...
					switch( strtoupper( $sageresult['Status'] ) ) {
		                ...
		                case 'AUTHENTICATED':
		                	...
		                	if( isset( $payment_form["opayo_card_token"] ) ) {

		                		if( $payment_form["opayo_card_token"] === 'new' ) {
									// Maybe save new token
									$this->save_token( 
											$data['Token'], 
											$payment_form["opayo_card_type"], 
											$payment_form["opayo_masked_card"], 
											$payment_form["opayo_card_expiry_mon"], 
											$payment_form["opayo_card_expiry_year"] 
										);
								}
