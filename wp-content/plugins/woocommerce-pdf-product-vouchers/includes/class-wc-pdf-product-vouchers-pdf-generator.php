...
class WC_PDF_Product_Vouchers_PDF_Generator {
	...
	public static function generate_voucher_pdf( WC_Voucher $voucher, $save = true ) {
		...
		if ( self::is_readable_local_file_path( $voucher->get_image_path() ) ) {
		...
		if ( $voucher->get_additional_image_id() && self::is_readable_local_file_path( $voucher->get_additional_image_path() ) ) {
		...
		if ( $voucher->get_logo_id() && self::is_readable_local_file_path( $voucher->get_logo_path() ) ) {
	 ...
	 * @return bool
	 */
	public static function is_readable_local_file_path( $path ) {

		$protocol = strstr( $path, '://', true );

		return ( ! $protocol || 'file' === $protocol ) && is_readable( $path );
	}
