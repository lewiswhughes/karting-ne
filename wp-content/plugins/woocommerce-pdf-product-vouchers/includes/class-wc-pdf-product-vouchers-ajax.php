	...
	public function redeem_voucher() {
		...
		if ( $voucher_id > 0 ) {
			...
			try {
				...
				// if the incorrect separator is used, raise a warning
				if (    ( '.' === $decimal_sep && false !== strpos( $amount, ',' ) )
				     || ( ',' === $decimal_sep && false !== strpos( $amount, '.' ) ) ) {

					wp_send_json_error( array(
						/* translators: Placeholder: %s - decimal separator (e.g. ',' or '.') */
						'message' => sprintf( __( 'Please enter the amount to redeem in monetary decimal (%s) format without thousand separators and currency symbols.', 'woocommerce-pdf-product-vouchers' ), $decimal_sep ),
					) );

				// replace a monetary separator with a floating point separator
				} elseif ( '.' !== $decimal_sep && false !== strpos( $amount, $decimal_sep ) ) {
