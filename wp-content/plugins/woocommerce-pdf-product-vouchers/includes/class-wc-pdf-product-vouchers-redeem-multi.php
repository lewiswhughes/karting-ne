...
class WC_PDF_Product_Vouchers_Redeem_Multi implements WC_PDF_Product_Vouchers_Redeem {
	...
	public function redeem( $voucher, $amount ) {
		...
		if ( $amount > 0 ) {
			...
			$message      = sprintf( $message_text, $voucher->get_voucher_number(), wc_price( $amount ), '<br />', wc_price( $remaining ) );
			...
			$message      = sprintf( $message_text, $voucher->get_voucher_number(), '<br />', wc_price( $remaining ), '<a href="' . get_edit_post_link( $voucher->get_id() ) . '">', '</a>' );
		}
